...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2022/04/02 20:46:11 by lkavalia          #+#    #+#             */
/*   Updated: 2022/04/04 19:24:45 by lkavalia         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*get_next_line(int fd)
{
	...
	while (size > 0)
	{
		temp[size] = '\0';
		if (!hold[fd])
			hold[fd] = ft_strdup_f(temp);
		else
			hold[fd] = ft_strjoin_f(hold[fd], temp);
		if (ft_strchr(temp, '\n'))
			break ;
		size = read(fd, temp, BUFFER_SIZE);
	}
	final = save_till_n_f(hold[fd]);
	hold[fd] = save_after_n_f(hold[fd]);
	free(temp);
	return (final);
}
